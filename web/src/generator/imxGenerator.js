...
function parseCardTypical(text) {
  ...
  for (const chunk of String(text || "").split(";")) {
    ...
    const m = raw.match(/^([^:=]+):=(.*)$/);
    if (!m) continue;
    out.set(m[1].trim().toUpperCase(), m[2].trim().replace(/^"|"$/g, ""));
  }
  return out;
}

function mapIODelimiter(ioFormat) {
  const s = String(ioFormat || "").toUpperCase();
  let out = s.includes("INPUT") ? "I" : s.includes("OUTPUT") ? "O" : "X";
  out += s.includes("DOT") ? "." : s.includes("SLASH") ? "/" : "|";
  return out;
}
...
export function generateIMX({ tables, siteId, options = {} }) {
  ...
  for (const card of rackForSite) {
    ...
    const addressDelimiter = mapIODelimiter(ioFormat);
        ...
        for (let xRung = 0; xRung <= xRungMax; xRung += 1) {
          ...
          if (ioByAddress.has(ioAddress)) {
            ...
            for (const side of ["Left", "Right"]) {
              ...
              append(endDeviceRung, makeMoNode(`${macroName}_${xRung + 1}${side}`, macroName, false, {
